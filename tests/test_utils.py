...
from fastlbp import get_p_for_r, get_radii
...
class TestFastlbpUtils(unittest.TestCase):
    def test_get_patch(self):
        ...
        self.assertTrue(
            (
                get_patch(data, patchsize, r, c)
                == data[(r * patchsize) : ((r + 1) * patchsize), (c * patchsize) : ((c + 1) * patchsize)]
            ).all(),
            "invalid read",
        )
    ...
    def test_histogram_reduction(self):
        ...
        for P in [14, 20, 27, 30]:
            with self.subTest(P=P, method="minimal"):
                ...
                self.assertTrue(
                    (test_reduced_hist == true_reduced_hist).all(),
                    f"\n true: {true_reduced_hist} \n test: {test_reduced_hist}",
                )
