...
from fastlbp import get_p_for_r, run_fastlbp
...
class TestFastLBP(unittest.TestCase):
    ...
    def test_small(self):
        ...
        self.assertFalse(
            os.path.exists(expected_output_abspath),
            f"Output file {expected_output_abspath} already exists. Initial cleanup somehow failed.",
        )
    ...
    def test_basic_cache_idempotency(self):
        ...
        self.assertFalse(
            os.path.isdir("./data/tmp"), f"Tmp directory {tmpabspath} already exists. Initial cleanup somehow failed."
        )
